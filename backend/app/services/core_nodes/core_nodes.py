...
def RAG_tool(state:State):
    ...
    response = retriver.get_context(state['refined_query'], top_k=6)
...
def classify_intent(state: State):
    ...
    system_prompt = SystemMessage(content=(
            ...
            "\n"
            "=== REFINED_QUERY GENERATION RULE ===\n"
            "For 'remainder' or 'general' intents: The refined_query should be a simple, basic summary of the user's current request.\n"
            "For 'RAG' intents ONLY: The refined_query MUST be as in-depth and detailed as possible to maximize vector database retrieval. "
            "You MUST strictly analyze the previous conversation context to resolve any pronouns (e.g., 'it', 'this process', 'that course'), "
            "carry over the ongoing topic, and merge earlier constraints with the user's current question. Synthesize the relevant conversation history "
            "into a single, highly detailed, standalone search query. Do NOT apply this deep contextual expansion to reminders.\n"
            "\n"
            "=== UPDATE/DELETE CONTEXT RULE (FOR REMINDERS ONLY) ===\n"
            ...
            "This is the ONLY exception to the current-message-only rule for reminders. "
